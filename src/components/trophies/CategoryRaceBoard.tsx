          ...
          <BrutalCard key={category.id} className="p-0">
            <div className="flex items-center justify-between gap-2 border-b border-white/10 px-2 py-1.5">
              <div className="min-w-0">
                ...
                <p className="mt-0.5 truncate text-[10px] font-semibold uppercase leading-none tracking-tighter text-zinc-500">
                  {leader ? `Leader · ${leader.tiktokerName}` : "En attente"}
                </p>
              </div>
              <span className="gold-pill shrink-0">
            ...
            <div className="divide-y divide-white/10">
              {rows.length === 0 ? (
                <div className="px-2 py-2 text-center text-xs font-semibold text-zinc-500">
                  Aucun nommé pour l&apos;instant
                </div>
              ) : (
                rows.map((row, index) => (
                  <motion.div
                  ...
                  >
                    <div className="grid grid-cols-[1rem_2rem_minmax(0,1fr)_auto] items-center gap-2">
                      <p className="rank-number text-[1.1rem] leading-none text-champagne">
                        {index + 1}
                      </p>
                      <div className="relative h-8 w-8 overflow-hidden rounded-full border border-champagne/45 bg-champagne/10">
                        {row.avatarUrl ? (
                          ...
                          <span className="flex h-full w-full items-center justify-center text-[9px] font-black text-champagneSoft">
                            {initialsFor(row.tiktokerName)}
                          </span>
                        )}
                      </div>
                      <div className="min-w-0">
                        <p className="truncate text-xs font-black leading-none tracking-tighter text-white">
                          @{row.tiktokerName}
                        </p>
                        <p className="mt-0.5 truncate text-[9px] font-semibold uppercase leading-none tracking-tighter text-zinc-500">
                          {row.acceptedDossiers}/{row.totalDossiers} nominés ·{" "}
                          {row.votes} notes ·{" "}
                    ...
                    <div className="mt-1 grid grid-cols-[minmax(0,1fr)_7rem_2.25rem] items-end gap-2 pl-[3rem]">
                      ...
                      <p className="text-right text-[10px] font-black leading-none tracking-tighter text-champagneSoft">
                        {row.successRate}%
                      </p>
                    </div>
                  </motion.div>
